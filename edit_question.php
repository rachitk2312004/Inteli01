...
$question_sql = "SELECT * FROM quiz_questions WHERE id = ? AND quiz_id = ?";
$question_stmt = mysqli_prepare($con, $question_sql);
mysqli_stmt_bind_param($question_stmt, 'ii', $question_id, $quiz_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    mysqli_stmt_bind_param($stmt, 'sssssssii', 
        ...
        $correct_option, $difficulty, $question_id, $quiz_id);
    
    if (mysqli_stmt_execute($stmt)) {
        $success = "Question updated successfully!";
    } else {
        $error = "Error updating question: " . mysqli_error($con);
    }
...
<body>
    ...
    <div class="container">
        <h1>Edit Question: <?= htmlspecialchars($quiz['title']) ?></h1>
